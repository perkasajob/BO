...
frappe.ui.form.on('DPPUv1v2', {
	setup: function(frm) {
		frappe.db.get_doc('DPPUv1v2 Settings')
			.then(settings => {
  ...
  onload: function(frm){
	...
	before_workflow_action: async function(frm){
		...
		if(frm.doc.workflow_state == "JV" && frm.doc.note.length < 5)
...
function getBookStatus(frm){
	frappe.call({
		method: "bo.bo.doctype.dppuv1.dppuv1.get_book_status",
		args: {
...
function bookDx(frm, check){
	...
	frm.states.show_actions()
	frappe.call({
		method: "bo.bo.doctype.dppuv1.dppuv1.book_transfer",
		args: {
			"docname": frm.doc.name,
			"check": check
		},
...
function checkBookAdvDx(frm, check){
	return frappe.call({
		method: "bo.bo.doctype.dppuv1.dppuv1.adv_transfer",
		args: {
			"docname": frm.doc.name,
			"check": check
		},
		callback: function(r) {
			if (r.message) {
				...
				} else if(r.message.status == "Jml Ccln is empty, No Adv DPPUv1v2"){
					frappe.throw("Jml Ccln is empty, No Adv DPPUv1v2", "Adv")
					frappe.validated = false;
				} else if(r.message.status == "No Book Record"){
					frappe.validated = false;
					frm.disable_save();
		...
		callback: function(r) {
			if (r.message) {
				if(r.message.status == "Success"){
					...
					frappe.msgprint("Jml Ccln is empty, No Adv DPPUv1v2", "Adv")
					frappe.validated = false;
				} else if(r.message.status == "No Book Record"){
...
function set_refund_btn(frm){
    if((frappe.user.has_role("ARCO") || frappe.user.has_role("Accounts Manager")) && frm.doc.amount_refund ){
        frm.add_custom_button(__('R fun'), function(){
			frappe.call({
				method: 'bo.bo.doctype.dppuv1.dppuv1.refund',
				args: {
					"docname": frm.doc.name
				},
				callback: function(r) {
...
function check_state_warning(frm){
	if(frappe.user.has_role("CSD") || frappe.user.has_role("Accounts Manager")|| frappe.user.has_role("Accounts User")){
		frappe.db.get_list("DPPUv1v2", {filters:{"dm_user":frm.doc.dm_user,"workflow_state" : ['in',["DM Received","Refund"]]},fields: ["workflow_state","name","number"], limit: 20}).then((d)=>{
			if(d.length > 0){
				var str = ''
				var sum = 0
				d.forEach(e => {
				...
				if (d.length > 3){
					str = '<table class="table" id="dppuv1-records" style="color:red"><tbody>'+str+ '<tr><td></td><td>Sum:</td><td>'+sum+'</td></tr></tbody></table>'
					frappe.msgprint({
						"title": "Not Eligible",
						"message": str,
						"indicator": "red"
					...
					str = '<table class="table" id="dppuv1-records"><tbody>'+str+ '<tr><td></td><td>Sum:</td><td>'+sum+'</td></tr></tbody></table>'
					frappe.msgprint(str, "Active DPPUv1v2")
				}
				// $(".form-page").prepend(str)
			} else{
				$("#dppuv1-records").remove()
			}
		})
	}
}
