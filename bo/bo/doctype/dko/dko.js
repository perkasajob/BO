...
function bookDx(frm, check){
	...
	frm.states.show_actions()
	frappe.call({
		method: "bo.bo.doctype.dko.dko.book_transfer",
		args: {
...
function bookAdvDx(frm, check){
	...
	frm.states.show_actions()
	frappe.call({
		method: "bo.bo.doctype.dko.dko.adv_transfer",
		args: {
			"docname": frm.doc.name,
			"check": check
		},
		callback: function(r) {
			if (r.message) {
				...
				} else if(r.message.status == "Jml Ccln is empty, No Adv DKO"){
					frappe.msgprint("Jml Ccln is empty, No Adv DKO", "Adv")
				} else if(r.message.status == "No Book Record"){
...
function set_refund_btn(frm){
    if((frappe.user.has_role("ARCO") || frappe.user.has_role("Accounts Manager")) && frm.doc.amount_refund ){
        frm.add_custom_button(__('R fun'), function(){
			frappe.call({
				method: 'bo.bo.doctype.dko.dko.refund',
				args: {
					"docname": frm.doc.name
				},
				callback: function(r) {
...
function check_state_warning(frm){
	if(frappe.user.has_role("CSD") || frappe.user.has_role("Accounts Manager")|| frappe.user.has_role("Accounts User")){
		frappe.db.get_list("DKO", {filters:{"dm_user":frm.doc.dm_user,"workflow_state" : ['in',["DM Received","Refund"]]},fields: ["workflow_state","name","number"], limit: 20}).then((d)=>{
			if(d.length > 0){
				var str = ''
				var sum = 0
				d.forEach(e => {
				...
				if (d.length > 3){
					str = '<table class="table" id="dko-records" style="color:red"><tbody>'+str+ '<tr><td></td><td>Sum:</td><td>'+sum+'</td></tr></tbody></table>'
					frappe.msgprint({
						"title": "Not Eligible",
						"message": str,
						"indicator": "red"
					...
					str = '<table class="table" id="dko-records"><tbody>'+str+ '<tr><td></td><td>Sum:</td><td>'+sum+'</td></tr></tbody></table>'
					frappe.msgprint(str, "Active DKO")
				}
