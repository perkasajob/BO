		...
		frm.call('parseXLS').then((res) => {
			if(res != undefined){
				if(res.message.data){
					...
					let total_col_nr = 0
					const colReTotal = RegExp('^Sum of|^Total','g');
					total_col_nr = res.message.data[nrow_column].findIndex((d, idx)=>{return colReTotal.test(d)})

					var columns = [{name: 'JABATAN', width:400, editable: false, format: formatFileName}].concat(res.message.data[nrow_column].slice(5).map(n=>{return {name: n, width:150}}))
