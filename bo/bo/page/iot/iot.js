frappe.pages['iot'].on_page_load = function(wrapper) {
	// var page = frappe.ui.make_app_page({
	// 	parent: wrapper,
	// 	title: 'IoT',
	// 	single_column: true
	// });
	frappe.iot = new IoT(wrapper);

	$(wrapper).bind('show', ()=> {
		// Get which leaderboard to show
		let doctype = frappe.get_route()[1];
		frappe.iot.show_leaderboard(doctype);
	});
}
...
class IoT {
	...
	get_leaderboard_config() {
		...
		frappe.xcall("frappe.desk.page.leaderboard.leaderboard.get_leaderboard_config").then(config => {
			this.iot_config = config;
			for (let doctype in this.iot_config) {
				this.doctypes.push(doctype);
				this.filters[doctype] = this.iot_config[doctype].fields.map(field => {
	...
	make() {
		...
		this.$graph_area = this.$container.find(".iot-graph");
	...
	render_selected_doctype() {
		...
		this.$sidebar_list.on("click", "li", (e)=> {
			...
			if (this.iot_config[this.options.selected_doctype].company_disabled) {
	...
	show_leaderboard(doctype) {
		if (this.doctypes.length) {
			...
			this.$search_box.find(".iot-search-input").val("");
			// frappe.set_route("leaderboard", this.options.selected_doctype);
		}
	}
	...
	get_leaderboard(notify) {
		...
		frappe.call(
			this.iot_config[this.options.selected_doctype].method,
			{
			...
			new frappe.Chart(".iot-graph", args);
	...
	get_leaderboard_data(me, res) {
		if (res && res.message.length) {
			me.message = null;
			me.$container.find(".iot-list").html(me.render_list_view(res.message));
			frappe.utils.setup_search($(me.parent), ".list-item-container", ".list-id");
		} else {
			me.$graph_area.hide();
			me.message = __("No items found.");
			me.$container.find(".iot-list").html(me.render_list_view());
		}
	}
	...
	get_item_html(item, index) {
		const fields = this.iot_config[this.options.selected_doctype].fields;
