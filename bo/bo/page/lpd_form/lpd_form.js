frappe.provide("bo.lpd");

frappe.pages['lpd-form'].on_page_load = function(wrapper) {	
	new bo.lpd.form(wrapper);
}

bo.lpd.form = class LPD {
	constructor(wrapper) {		
	...
	make() {
		...
		const empty_state = __("Add Items")
...
bo.lpd.LPDUpload = class LPDUpload {
	constructor(parent) {
		this.parent = parent;
		this.data = [];
	...
	make() {
		...
		new frappe.ui.FileUploader({
			method: 'bo.bo.lpd_form.lpd_upload.upload_lpd_form',
			allow_multiple: 0,
...
bo.lpd.bankTransactionSync = class bankTransactionSync {
	constructor(parent) {
		this.parent = parent;
		this.data = [];
...
erpnext.accounts.ReconciliationTool = class ReconciliationTool extends frappe.views.BaseList {
	...
	render() {
		...
		me.data.map((value) => {
			...
			new bo.lpd.ReconciliationRow(row, value);
		})
	}
...
bo.lpd.ReconciliationRow = class ReconciliationRow {
